package com.weyko.shops.log;

import android.content.Context;
import android.os.Environment;
import android.util.Log;

import com.weyko.shops.util.FileUtils;
import com.weyko.shops.util.Network;
...
public class LogUtil {
	...
	public static void writeLogtoFile(String tag, String text){
		if(isDebug){
			...
			createCacheDir();
			String time = TimeUtils.getCurrentTime(TimeUtils.TimeFormatType.TIME_FOEMAT_STANDARD);
	        String needWriteMessage = time + "    " + tag + "    " + text;
	        File file = new File(logCacheDir, "mx_log_" + TimeUtils.getCurrentTime(TimeUtils.TimeFormatType.TIME_FOEMAT_Y_M_D)+".txt");
	        try {  
	...
	* @return void
	...
	public static void writeRequestLogtoFile(String text){
		if(isDebug){
			...
			if (openRequestLog){
				createCacheDir();
		        String needWriteMessage = text+"\r\n";
		        File file = new File(logCacheDir, requestTag + TimeUtils.getCurrentTime(TimeUtils.TimeFormatType.TIME_FOEMAT_Y_M_D)+".txt");
		        try {  
		            FileWriter filerWriter = new FileWriter(file, true);
		            BufferedWriter bufWriter = new BufferedWriter(filerWriter);
		            bufWriter.write(needWriteMessage);  
	...
	public static synchronized String timeAndNetType(Context context){
		String str=TimeUtils.getCurrentTime(TimeUtils.TimeFormatType.TIME_FOEMAT_STANDARD);
		if(context!=null){
			str += "|"+ Network.getConnectedType(context);
			System.gc();
		}
		str += "|";
		return str;
